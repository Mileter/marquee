...
void UpdateErrorList() {
    ...
    for (int i = 0; i < g_editor->errorCount; i++) {
        ...
        switch (g_editor->errors[i].severity) {
            case 0: wcscpy(severityStr, L"Info"); break;
            case 1: wcscpy(severityStr, L"Warning"); break;
            case 2: wcscpy(severityStr, L"Error"); break;
            default: wcscpy(severityStr, L"Unknown"); break;
        }
...
void UpdateGutterAndRect() {
    ...
    SendMessageW(g_editor->hwndEdit, EM_SETRECTNP, 0, (LPARAM)&formatRect);
...
LRESULT CALLBACK EditControlProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT: {
        ...
        LRESULT result = CallWindowProcW(g_editor->originalEditProc, hwnd, uMsg, wParam, lParam);
        ...
        if (hdc) {
            ...
            RECT gutterRect = {0, 0, g_editor->gutterWidth, clientRect.bottom};
            FillRect(hdc, &gutterRect, CreateSolidBrush(RGB(245, 245, 245)));
            ...
            int visibleLines = (clientRect.bottom - clientRect.top) / g_editor->lineHeight + 2;
            ...
            for (int i = 0; i < visibleLines && (firstVisibleLine + i) < totalLines; i++) {
                ...
                int yPos = baseYPos + (i * g_editor->lineHeight);
                
                /* Only draw if the line is within the visible client area */
                if (yPos >= 0 && yPos + g_editor->lineHeight <= clientRect.bottom) {
                    RECT lineRect = {
                        2, 
                        yPos, 
                        g_editor->gutterWidth - 4, 
                        yPos + g_editor->lineHeight
                    };
                    
                    DrawTextW(hdc, lineText, -1, &lineRect, DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
                }
            }
            ...
            MoveToEx(hdc, g_editor->gutterWidth - 1, 0, NULL);
            LineTo(hdc, g_editor->gutterWidth - 1, clientRect.bottom);
    ...
    case WM_LBUTTONDOWN: {
        ...
        if (xPos < g_editor->gutterWidth) {
            ...
            int charIndex = (int)SendMessageW(hwnd, EM_CHARFROMPOS, 0, MAKELPARAM(pt.x, pt.y));
            if (charIndex >= 0) {
                int lineIndex = (int)SendMessageW(hwnd, EM_LINEFROMCHAR, charIndex, 0);
                int lineStart = (int)SendMessageW(hwnd, EM_LINEINDEX, lineIndex, 0);
                int lineLength = (int)SendMessageW(hwnd, EM_LINELENGTH, lineStart, 0);
                
                /* Select the entire line */
                SendMessageW(hwnd, EM_SETSEL, lineStart, lineStart + lineLength);
            }
            return 0;
        }
        break;
    ...
    case WM_MOUSEWHEEL:
        ...
        CallWindowProcW(g_editor->originalEditProc, hwnd, uMsg, wParam, lParam);
    ...
    return CallWindowProcW(g_editor->originalEditProc, hwnd, uMsg, wParam, lParam);
}
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (line[i] == L'`') {
            ...
            if (!PushBacktick(&stack, i, hasColorSpec)) {
                AddValidationError(lineNum, L"Too many nested color specifications (maximum 10)", 2);
            }
...
void ValidateFile() {
    ...
    int lineNum = 1;
    int segmentCount = 0;
    BOOL hasLPS = FALSE, hasSW = FALSE, hasSH = FALSE, hasSC = FALSE, hasSD = FALSE;
        ...
        if (c == L'\r') {
            ...
            if (line[0] != L'/' && len > 0) {
                ...
                /* OPTIONAL FLAGS */
                    ...
                    if (hasTPF) AddValidationError(lineNum, L"Duplicate TPF command", 1); /* Warning for optional */
                    hasTPF = TRUE;
                    int value = _wtoi(&line[4]);
                    if (value <= 0) AddValidationError(lineNum, L"TPF (millis per frame) must be positive", 2);
                    if (value < 16) AddValidationError(lineNum, L"TPF below 16ms may cause performance issues", 1); /* Warning */
                } else if (wcsncmp(line, L"PM", 2) == 0) {
                    if (hasPM) AddValidationError(lineNum, L"Duplicate PM command", 1); /* Warning for optional */
                    hasPM = TRUE;
                    int value = _wtoi(&line[3]);
                    if (value <= 0) AddValidationError(lineNum, L"PM (pixel movement per frame) must be positive", 2);
                    if (value > 20) AddValidationError(lineNum, L"PM above 20 pixels may scroll too fast", 1); /* Warning */
    ...
    if (g_editor->errorCount == 0) {
        ...
        swprintf(statusMsg, 64, L"Validation failed - %d errors found", g_editor->errorCount);
        ...
        if (file) {
            ...
            if (size >= 2) {
                ...
                if (bom[0] == 0xFF && bom[1] == 0xFE) {
                    ...
                    if (remainingSize > 0) {
                        wchar_t* wideBuffer = malloc(remainingSize + sizeof(wchar_t));
                        if (wideBuffer) {
                            size_t bytesRead = fread(wideBuffer, 1, remainingSize, file);
                            /* Null terminate */
                            wideBuffer[bytesRead / sizeof(wchar_t)] = L'\0';
                            SetWindowTextW(g_editor->hwndEdit, wideBuffer);
                            free(wideBuffer);
                        }
                    } else {
                    ...
                    if (buffer) {
                        ...
                        INT flags = IS_TEXT_UNICODE_STATISTICS | IS_TEXT_UNICODE_CONTROLS;
                        if (IsTextUnicode(buffer, size, &flags)) {
                            /* Treat as UTF-16 */
                            SetWindowTextW(g_editor->hwndEdit, (wchar_t*)buffer);
                        } else {
                            /* Convert from multibyte to wide chars */
                            int wideSize = MultiByteToWideChar(CP_UTF8, 0, buffer, -1, NULL, 0);
                            wchar_t* wideBuffer = malloc(wideSize * sizeof(wchar_t));
                            if (wideBuffer) {
...
void SaveFile(BOOL saveAs) {
    ...
    if (wideBuffer) {
        ...
        if (file) {            
            ...
            MessageBoxW(g_editor->hwndMain, L"Could not save file", L"Error", MB_OK | MB_ICONERROR);
        }
        free(wideBuffer);
    }
}
...
void LaunchPreview() {
    if (g_editor->isModified || wcslen(g_editor->currentFile) == 0) {
        int result = MessageBoxW(g_editor->hwndMain, 
            ...
            L"Preview", MB_YESNO | MB_ICONQUESTION);
        if (result == IDYES) {
            SaveFile(FALSE);
        } else {
            return;
    ...
    if (!CreateProcessW(NULL, cmdLine, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
        MessageBoxW(g_editor->hwndMain, L"Could not launch renderer. Make sure renderer.exe is in the same directory.", 
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        HMENU hMenu = LoadMenuW(g_editor->hInstance, MAKEINTRESOURCEW(IDR_MAINMENU));
        if (hMenu) {
            SetMenu(hwnd, hMenu);
        }
        
        /* Create font for editor */
        g_editor->hFont = CreateFontW(-12, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
                                DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                                DEFAULT_QUALITY, FIXED_PITCH | FF_MODERN, L"MingLiU");
        if(!g_editor->hFont)
            g_editor->hFont = CreateFontW(-12, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
                                DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
        ...
        /* Create edit control */
        g_editor->hwndEdit = CreateWindowExW(
            WS_EX_CLIENTEDGE,
            L"EDIT",
            L"",
            WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_MULTILINE | ES_AUTOVSCROLL | ES_AUTOHSCROLL | ES_NOHIDESEL,
            10, 10, 800, 400,
            hwnd, (HMENU)IDC_EDIT_MAIN, g_editor->hInstance, NULL
        );
        
        /* Subclass the edit control to handle custom painting */
        g_editor->originalEditProc = (WNDPROC)SetWindowLongPtrW(g_editor->hwndEdit, GWLP_WNDPROC, (LONG_PTR)EditControlProc);
        
        /* Create error list */
        g_editor->hwndErrorList = CreateWindowExW(
            WS_EX_CLIENTEDGE,
            WC_LISTVIEWW,
            L"",
            WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_SINGLESEL,
            10, 450, 800, 150,
            hwnd, (HMENU)IDC_LIST_ERRORS, g_editor->hInstance, NULL
        );
        ...
        SendMessageW(g_editor->hwndEdit, WM_SETFONT, (WPARAM)g_editor->hFont, TRUE);
        SendMessageW(g_editor->hwndErrorList, WM_SETFONT, (WPARAM)g_editor->hFont, TRUE);
        
        /* Create status bar */
        g_editor->hwndStatus = CreateWindowW(
            STATUSCLASSNAMEW, L"Ready",
            WS_CHILD | WS_VISIBLE,
            0, 0, 0, 0,
            hwnd, (HMENU)IDC_STATUS, g_editor->hInstance, NULL
        );
    ...
    case WM_SIZE: {
        ...
        SetWindowPos(g_editor->hwndEdit, NULL, 10, 10, 
            rect.right - 20, (rect.bottom - statusHeight) / 2 - 20, 
            SWP_NOZORDER);
        
        /* Resize error list - takes up bottom half */
        SetWindowPos(g_editor->hwndErrorList, NULL, 10, (rect.bottom - statusHeight) / 2 + 10,
            rect.right - 20, (rect.bottom - statusHeight) / 2 - 20,
            SWP_NOZORDER);
    ...
    case WM_CLOSE:
        if (g_editor->isModified) {
            int result = MessageBoxW(hwnd, L"Save changes before closing?", L"Marquee Editor", 
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow) {
    ...
    /* Create window */
    g_editor->hwndMain = CreateWindowExW(
        0,
        L"MarqueeEditor",
    ...
    while (GetMessage(&msg, NULL, 0, 0)) {
        ...
        if (!hAccel || !TranslateAcceleratorW(g_editor->hwndMain, hAccel, &msg)) {
